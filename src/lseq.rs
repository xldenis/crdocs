...
fn interval_between(a: &Identifier, b: &Identifier, depth: usize, orig_base: u32) -> usize {
...
fn test_different_len_paths() {
    ...
    assert_eq!(interval_between(&x, &y, 2, 3), 15);
}
...
fn test_long_paths() {
    ...
    assert_eq!(interval_between(&x, &y, 3, 3), 30 + 32 * 32);
}
...
fn alloc(p: Identifier, q: Identifier) -> Identifier { // make that a bigint
    ...
    while (interval < 1) {
        depth += 1;
        // search for a depth where we can finally insert our element
        interval = interval_between(&p, &q, depth, INITIAL_BASE);
    }

    let step = usize::min(BOUNDARY, interval);
    let mut rng = rand::thread_rng();
    let shift = rng.gen_range(0, step) + 1;

    if (depth % 2 == 0) { // hash
        let mut new_ident = p.path[0..depth].to_owned();
        new_ident[depth - 1] += shift;
        return Identifier { path : new_ident }
        // return prefix(p, depth) + shift;
    } else {
        let mut new_ident = q.path[0..depth].to_owned();
        new_ident[depth - 1] -= shift;
...
#[test]
fn test_alloc() {
    let a = Identifier { path: vec![1], };
    let b = Identifier { path: vec![3], };

    assert_eq!(alloc(a, b), Identifier { path: vec![2]});


    let c = Identifier { path: vec![1,0,1], };
    let d = Identifier { path: vec![1,0,3], };

    assert_eq!(alloc(c, d), Identifier { path: vec![1,0,2]});
}
